...
export function Analytics() {
  ...
  return (
    <div className="w-full space-y-6 pb-8 text-[#f7f8f8]">
      ...
      {/* A/B Testing: AI vs Holdout Performance (Incremental ROI) */}
      <div className="bg-[#13161c]/60 border border-[#1e2025]/80 rounded-2xl p-6 relative overflow-hidden group">
        {/* Decorative ambient background */}
        <div className="absolute inset-0 bg-gradient-to-br from-[#5e6ad2]/5 to-transparent opacity-50 pointer-events-none" />
        <div className="absolute -top-24 -right-24 w-64 h-64 bg-[#5e6ad2]/10 blur-[100px] rounded-full pointer-events-none transition-opacity duration-700 group-hover:opacity-100" />
        
        <div className="relative z-10">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              <h3 className="flex items-center text-lg font-bold text-[#f7f8f8]">
                <Sparkles className="w-5 h-5 text-[#5e6ad2] mr-2" />
                AI Smart-Yield vs. Standard Holdout
              </h3>
              <p className="text-sm text-[#8a8f98] mt-1">Measuring true incremental revenue generated by RecoverIQ vs traditional static recovery.</p>
            </div>
            
            <div className="flex items-center gap-3">
              <div className="flex items-center bg-[#1e2025] border border-[#34343a] rounded-lg px-3 py-1.5">
                <span className="flex items-center text-xs font-semibold text-emerald-400">
                  <TrendingUp className="w-3.5 h-3.5 mr-1.5" />
                  +{(experimentMetrics?.incrementalLiftPercent || 0).toFixed(1)}% Lift
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Treatment Group (AI) */}
            <div className="bg-[#1a1d24] border border-[#2a2d35] rounded-xl p-5 shadow-inner">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                  <div className="w-2.5 h-2.5 rounded-full bg-[#5e6ad2] mr-2 shadow-[0_0_8px_#5e6ad2]" />
                  <span className="text-sm font-semibold text-[#f7f8f8]">RecoverIQ Group</span>
                </div>
                <span className="text-xs text-[#8a8f98] flex items-center"><Users className="w-3.5 h-3.5 mr-1" /> {experimentMetrics?.treatmentCount || 0}</span>
              </div>
              
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-xs mb-1.5">
                    <span className="text-[#8a8f98]">Recovered Amount</span>
                    <span className="text-emerald-400 font-bold">{formatCurrency(experimentMetrics?.treatmentRecoveredAmount || 0)}</span>
                  </div>
                  <div className="h-2 w-full bg-[#13161c] rounded-full overflow-hidden">
                    <div className="h-full bg-[#5e6ad2] rounded-full transition-all duration-700" style={{ width: `${experimentMetrics?.treatmentRecoveryRate || 0}%` }} />
                  </div>
                </div>
                
                <div className="flex justify-between items-center text-xs pt-2 border-t border-[#2a2d35]">
                  <span className="text-[#8a8f98]">Recovery Rate</span>
                  <span className="text-[#f7f8f8] font-mono font-semibold">{(experimentMetrics?.treatmentRecoveryRate || 0).toFixed(1)}%</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#1a1d24] border border-[#2a2d35] rounded-xl p-5 shadow-inner">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                  <div className="w-2.5 h-2.5 rounded-full bg-[#8a8f98] mr-2" />
                  <span className="text-sm font-semibold text-[#f7f8f8]">Holdout (Control)</span>
                </div>
                <span className="text-xs text-[#8a8f98] flex items-center"><Users className="w-3.5 h-3.5 mr-1" /> {experimentMetrics?.holdoutCount || 0}</span>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-xs mb-1.5">
                    <span className="text-[#8a8f98]">Recovered Amount</span>
                    <span className="text-[#f7f8f8] font-bold">{formatCurrency(experimentMetrics?.holdoutRecoveredAmount || 0)}</span>
                  </div>
                  <div className="h-2 w-full bg-[#13161c] rounded-full overflow-hidden">
                    <div className="h-full bg-[#8a8f98] rounded-full transition-all duration-700" style={{ width: `${experimentMetrics?.holdoutRecoveryRate || 0}%` }} />
                  </div>
                </div>
                
                <div className="flex justify-between items-center text-xs pt-2 border-t border-[#2a2d35]">
                  <span className="text-[#8a8f98]">Recovery Rate</span>
                  <span className="text-[#f7f8f8] font-mono font-semibold">{(experimentMetrics?.holdoutRecoveryRate || 0).toFixed(1)}%</span>
                </div>
              </div>
            </div>

            {/* Incremental ROI Highlight */}
            <div className="bg-gradient-to-br from-emerald-500/10 to-[#5e6ad2]/10 border border-emerald-500/20 rounded-xl p-5 flex flex-col justify-center shadow-lg relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/10 blur-[50px] rounded-full pointer-events-none" />
              
              <div className="text-center z-10">
                <span className="text-xs font-semibold text-emerald-400 tracking-wider uppercase mb-1 block">Net Incremental Revenue</span>
                <div className="text-4xl font-bold font-mono text-[#f7f8f8] my-3 drop-shadow-[0_0_15px_rgba(16,185,129,0.3)]">
                  {formatCurrency(experimentMetrics?.netIncrementalRecovered || 0)}
                </div>
                <p className="text-xs text-[#8a8f98] max-w-[200px] mx-auto">
                  Pure profit generated that would have otherwise been lost.
                </p>
              </div>
            </div>
          </div>
