...
export function Dashboard() {
  ...
  return (
    <DarkGradientBg className="flex-1 min-h-0 text-[#f7f8f8]">
      ...
      {/* AI Revenue Recovery Callout Banner */}
      {atRiskAmount > 0 && (
        <Link
          to="/recovery"
          className="flex items-center gap-3 p-3.5 bg-gradient-to-r from-amber-500/10 via-cyan-500/5 to-violet-500/10 border border-amber-500/20 rounded-xl hover:border-amber-500/40 transition-all flex-shrink-0 group"
        >
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-amber-500/20 to-amber-600/20 border border-amber-500/20 flex items-center justify-center flex-none">
            <TrendingUp className="w-4 h-4 text-amber-400" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-amber-300">AI Revenue Recovery</span>
              <span className="flex items-center gap-1 text-xs text-emerald-400 bg-emerald-400/10 border border-emerald-400/20 rounded-full px-1.5 py-0.5">
                <span className="w-1 h-1 rounded-full bg-emerald-400 animate-pulse" />
                Active
              </span>
            </div>
            <div className="text-xs text-zinc-500 mt-0.5">
              Detecting revenue at risk · Razorpay-powered recovery workflows
            </div>
          </div>
          <div className="text-right flex-none">
            <div className="text-sm font-bold text-amber-400">
              {atRiskAmount >= 100000 ? `₹${(atRiskAmount / 100000).toFixed(1)}L` : `₹${atRiskAmount.toLocaleString('en-IN')}`}
            </div>
            <div className="text-xs text-zinc-500">at risk</div>
          </div>
          <ChevronRight className="w-4 h-4 text-zinc-500 group-hover:text-zinc-300 transition-colors flex-none" />
        </Link>
      )}
