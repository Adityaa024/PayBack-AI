...
## 📈 Proof of Yield: Dual-Denominator Evaluation

We do not merely assert AI recovery; we prove it mathematically. Following the rigorous standards of [`piyush2676/recoverx`](https://github.com/piyush2676/recoverx) and [`Ovais-Maker/razorpay-buildathon-recoup`](https://github.com/Ovais-Maker/razorpay-buildathon-recoup), PayBack-AI evaluates recovery against **two distinct denominators side-by-side**:
1. **Total Failed Debt (₹1,768,306.45)**: Traditional gross denominator (inflated with structurally unrecoverable funds like fraud, closed accounts, and >90-day statutory caps).
...
| **PayBack-AI Heuristic** | ₹1,768,306.45 | ₹945,618.25 | **53.48%** | **100.00%** | 1002 | ₹1,503.00 | ₹944,115.25 | **₹578,146.41** |
...
Modeled on `Ovais-Maker/razorpay-buildathon-recoup`, we evaluated our deterministic PolicyGuard heuristic against the LLM strategist on identical cases under identical guardrails. **The heuristic won in net yield (₹578,146.41 vs ₹520,507.02).**
- **Token Overhead**: The LLM incurred **₹343.00** in API token costs (~₹0.35/eval), eroding unit economics on micro-invoices.
- **Reasoning Hesitation**: On late-stage overdue (75–90 days), the LLM occasionally recommended manual staff review, whereas PolicyGuard deterministically executed compliant recovery touches up to the 90-day statutory boundary.
- **Offline Replay Mode**: All LLM decisions are cached in `reports/llm_cache.json` (`--llm-offline`) so reviewers can verify all LLM figures locally without requiring external API keys.
...
│   │   ├── generate_llm_cache.py  # Offline LLM decision cache generator
│   │   ├── run_evaluation.py      # Empirical dual-denominator evaluation engine
...
├── reports/                       # Generated evaluation batches, llm_cache.json, evaluation.json
├── scripts/
...
├── EVALUATION.md                  # Auto-generated dual-denominator empirical report
└── FAILURES.md                    # Defect log and architectural post-mortems (9 entries)
```
