...
describe('MX Verifier Module', () => {
  ...
  describe('validateInboundDomainFormat', () => {
    it('accepts valid full domain names', () => {
      expect(validateInboundDomainFormat('reply.recoveriq.site')).toBe('reply.recoveriq.site');
    ...
    it('rejects root domains without dedicated subdomain', () => {
      expect(() => validateInboundDomainFormat('recoveriq.site')).toThrow(
        'is a root domain'
      );
  ...
  describe('verifyInboundMxForProvider', () => {
    describe('SendGrid Provider', () => {
      it('accepts domains with SendGrid MX records', async () => {
        ...
        const domain = await verifyInboundMxForProvider('reply.recoveriq.site', 'sendgrid');
        expect(domain).toBe('reply.recoveriq.site');
      });
      ...
      it('rejects domains with non-SendGrid MX records', async () => {
        ...
        await expect(
          verifyInboundMxForProvider('reply.recoveriq.site', 'sendgrid')
        ).rejects.toThrow(
          'The MX records for "reply.recoveriq.site" do not point to SendGrid'
        );
      });
    });
    ...
    describe('Resend Provider', () => {
      it('accepts domains with Resend MX records (inbound.resend.com)', async () => {
        ...
        const domain = await verifyInboundMxForProvider('reply.recoveriq.site', 'resend');
        expect(domain).toBe('reply.recoveriq.site');
      });
      ...
      it('rejects feedback-smtp bounce-only records because they cannot receive incoming debtor replies', async () => {
        ...
        await expect(
          verifyInboundMxForProvider('reply.recoveriq.site', 'resend')
        ).rejects.toThrow(
          'The MX records for "reply.recoveriq.site" do not point to Resend'
        );
      });
      ...
      it('throws clear error when no MX records exist at all', async () => {
        ...
        await expect(
          ...
          'No DNS MX records found for "reply.recoveriq.site"'
        );
      });
    });
  });
