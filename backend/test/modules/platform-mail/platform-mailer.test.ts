...
describe('PlatformMailer', () => {
  ...
  beforeEach(() => {
    ...
    mockResolver = {
      ...
      resolveSender: vi.fn().mockResolvedValue({
        fromEmail: 'no-reply@recoveriq.site',
        fromName: 'RecoverIQ',
      }),
    };
  });
  ...
  it('should resolve config and send team invitation email successfully', async () => {
    ...
    const result = await platformMailer.sendTeamInviteEmail('invited@example.com', 'https://recoveriq.site/invite#token=abc');
    ...
    expect(mockProviderInstance.send).toHaveBeenCalledWith({
      ...
      from: { name: 'RecoverIQ', email: 'no-reply@recoveriq.site' },
      subject: 'You have been invited to join RecoverIQ',
      html: expect.stringContaining('https://recoveriq.site/invite#token=abc'),
    });
  ...
  describe('EnvPlatformEmailConfigResolver', () => {
    ...
    it('resolves Resend platform email config from env without requiring webhooks', async () => {
      ...
      process.env.PLATFORM_FROM_EMAIL = 'auth@recoveriq.site';
      process.env.PLATFORM_FROM_NAME = 'RecoverIQ System';
      ...
      expect(sender).toEqual({
        fromEmail: 'auth@recoveriq.site',
        fromName: 'RecoverIQ System',
      });
    });
