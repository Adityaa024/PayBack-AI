...
export class DisputeService {
  ...
  setRecoveryService(recoveryService: any) {
  ...
  async processInboundEmail(params: {
    ...
    // Extract Promise-to-Pay (PTP) commitment from inbound email if present
    if (this.recoveryService && this.aimlService && invoice) {
      try {
        const ptpResult = await this.aimlService.callPromiseExtractor({
        ...
        if (ptpResult.has_promise && ptpResult.confidence > 0.5) {
          await this.recoveryService.createPTP({
            tenantId,
            invoiceId,
            promisedDate: ptpResult.promised_date || undefined,
            promisedAmount: ptpResult.promised_amount || undefined,
            currency: invoice.currency,
            aiExtractedText: ptpResult.extracted_text || emailBody.slice(0, 300),
            aiConfidence: ptpResult.confidence,
          });
          logger.info('ptp_extracted_from_inbound_email', {
            invoiceId,
            promisedDate: ptpResult.promised_date,
            confidence: ptpResult.confidence,
          });
        }
      } catch (ptpErr) {
        logger.warn('ptp_extraction_failed_non_fatal', { error: ptpErr, invoiceId });
      }
    }
