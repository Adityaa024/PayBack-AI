...
export class IntegrationService {
  ...
  async getSendgridSetupProgress(tenantId: string): Promise<SendgridSetupProgress> {
    ...
    const publicBaseUrl = config.PUBLIC_BASE_URL || process.env.PUBLIC_BASE_URL || 'https://www.recoveriq.site';
  ...
  async testResendKey(tenantId: string, toEmail: string): Promise<{ success: boolean; message: string }> {
    ...
    const { data, error } = await resend.emails.send({
      from: fromAddress,
      to: [toEmail],
      subject: 'RecoverIQ Test Email (Resend)',
      html: '<p>This is a test email confirming that your Resend integration with RecoverIQ is functioning properly.</p>',
