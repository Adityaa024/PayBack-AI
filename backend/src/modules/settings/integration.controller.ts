      ...
      if (!emailSent && this.communicationService) {
        try {
          await this.communicationService.send({
            tenantId,
            to: targetEmail,
            subject: 'RecoverIQ Mailbox Verification Code',
            html: `<div style="font-family: sans-serif; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px; max-width: 500px;">
              ...
              <p style="color: #475569;">Enter the following 6-digit OTP code in RecoverIQ Settings to verify ownership of <strong>${targetEmail}</strong>:</p>
