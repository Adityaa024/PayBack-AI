...
export class AuthService {
  ...
  async onboard(input: OnboardInput): Promise<{ pendingVerification: true } | AuthResult> {
    ...
    // In dev mode, or when email provider is not configured, skip OTP verification
    // and auto-create the user as verified immediately.
    const isDevMode = process.env.NODE_ENV === 'development' || process.env.SKIP_EMAIL_VERIFICATION === 'true';
    ...
    if (isDevMode) {
      ...
      const { user } = await this.userRepo.createTenantWithAdmin(
        ...
        { name: input.name, email: normalizedEmail, passwordHash, role: 'admin', emailVerified: true }
      );
      const token = this.signToken(user);
      return { user: this.stripSensitive(user), token } as AuthResult;
    }
  ...
  async initiateMfaSetup(userId: string): Promise<MfaSetupInitiateResult> {
    ...
    const otpauthUrl = totp.toURI({ label: user.email, issuer: 'RecoverIQ', secret });
  ...
  async verifyAndFetchUser(token: string): Promise<JwtPayload> {
    ...
    if (payload.userId === 'demo_admin') {
      return {
        userId: 'demo_admin',
        tenantId: payload.tenantId || 'tenant_demo_001',
        name: payload.name || 'Razorpay Judge',
        email: payload.email || 'judge@razorpay.com',
        role: 'admin',
      };
    }
