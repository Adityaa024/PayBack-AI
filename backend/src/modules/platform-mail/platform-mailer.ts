...
export class EnvPlatformEmailConfigResolver implements PlatformEmailConfigResolver {
  async resolveSender(): Promise<{ fromEmail: string; fromName: string }> {
    return {
      fromEmail: process.env.PLATFORM_FROM_EMAIL || 'no-reply@recoveriq.site',
      fromName: process.env.PLATFORM_FROM_NAME || 'RecoverIQ',
    };
  }
    ...
    return this.sendMail((sender) => ({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #334155;">
          ...
          <p style="font-size: 14px; line-height: 1.6;">Please enter the following 6-digit verification code in RecoverIQ to confirm that this inbox is active and able to receive customer replies:</p>
