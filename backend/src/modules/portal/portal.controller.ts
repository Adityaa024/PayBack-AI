...
export class PortalController {
  private recoveryService?: { trackAbandonmentSignal: (tenantId: string, invoiceId: string) => Promise<void> };
  ...
  setRecoveryService(recoveryService: { trackAbandonmentSignal: (tenantId: string, invoiceId: string) => Promise<void> }) {
  ...
  getInvoiceDetails = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      // Track checkout abandonment signal if invoice is unpaid
      if (this.recoveryService && invoice.paymentStatus !== 'Paid' && invoice.paymentStatus !== 'Written Off') {
        this.recoveryService.trackAbandonmentSignal(tenant.id, invoice.id).catch(() => {
          // non-fatal
        });
      }
