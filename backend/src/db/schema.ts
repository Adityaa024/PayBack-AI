...
export const recoveryStopReasonEnum = pgEnum('recovery_stop_reason', [
  'max_retries_reached',
  'legal_stop',
  'manual_override',
  'dlq_threshold',
...
/** One recovery workflow session per at-risk invoice */
export const recoverySessions = pgTable(
  'recovery_sessions',
  {
    ...
    invoiceId: varchar('invoice_id', { length: 36 })
      .notNull()
      .references(() => invoices.id, { onDelete: 'cascade' }),
    status: recoverySessionStatusEnum('status').notNull().default('active'),
    strategy: recoveryStrategyEnum('strategy').notNull().default('payment_link_refresh'),
    incidentLane: varchar('incident_lane', { length: 50 }).notNull().default('payment_degradation'),
    isHoldout: boolean('is_holdout').notNull().default(false),
    recoveryContract: jsonb('recovery_contract'),
    voiceScriptHinglish: text('voice_script_hinglish'),
    optedOut: boolean('opted_out').notNull().default(false),
    amountAtRisk: numeric('amount_at_risk', { precision: 14, scale: 2 }).notNull(),
  ...
  (table) => [
    index('recovery_sessions_tenant_status_idx').on(table.tenantId, table.status),
    index('recovery_sessions_invoice_id_idx').on(table.invoiceId),
    index('recovery_sessions_tenant_created_idx').on(table.tenantId, table.createdAt),
    // BUG-03 FIX: Prevent multiple active sessions for the same invoice
    uniqueIndex('recovery_sessions_active_uniq')
      .on(table.tenantId, table.invoiceId)
      .where(sql`${table.status} = 'active'`),
  ]
);

/** Each individual Razorpay payment link retry attempt */
export const paymentRetryAttempts = pgTable(
  'payment_retry_attempts',
  {
    ...
    communicationId: varchar('communication_id', { length: 36 })
      .references(() => communications.id, { onDelete: 'set null' }),
...
/** Debtor promise-to-pay records extracted by AI from inbound emails */
export const promiseToPay = pgTable(
  'promise_to_pay',
  {
    ...
    invoiceId: varchar('invoice_id', { length: 36 })
      ...
      .references(() => invoices.id, { onDelete: 'cascade' }),
    sessionId: varchar('session_id', { length: 36 })
      .references(() => recoverySessions.id, { onDelete: 'set null' }),
    detectedFromCommunicationId: varchar('detected_from_communication_id', { length: 36 })
      .references(() => communications.id, { onDelete: 'set null' }),
    promisedAmount: numeric('promised_amount', { precision: 14, scale: 2 }),
    promisedDate: date('promised_date', { mode: 'string' }),
    currency: varchar('currency', { length: 10 }).notNull().default('INR'),
    status: promiseToPayStatusEnum('status').notNull().default('pending'),
  ...
  (table) => [
    index('ptp_tenant_status_idx').on(table.tenantId, table.status),
    index('ptp_invoice_id_idx').on(table.invoiceId),
    index('ptp_promised_date_idx').on(table.promisedDate),
  ]
);

/** Portal view events without payment — checkout abandonment signals */
export const checkoutAbandonmentSignals = pgTable(
  'checkout_abandonment_signals',
  {
    id: varchar('id', { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
    tenantId: varchar('tenant_id', { length: 36 })
      .notNull()
    ...
    invoiceId: varchar('invoice_id', { length: 36 })
      .notNull()
      .references(() => invoices.id, { onDelete: 'cascade' }),
    portalViewedAt: timestamp('portal_viewed_at', { mode: 'date' }).notNull(),
    recoveryTriggered: boolean('recovery_triggered').notNull().default(false),
    recoveryTriggeredAt: timestamp('recovery_triggered_at', { mode: 'date' }),
    sessionId: varchar('session_id', { length: 36 })
      .references(() => recoverySessions.id, { onDelete: 'set null' }),
    createdAt: timestamp('created_at', { mode: 'date' })
      .notNull()
  ...
  (table) => [
    index('abandonment_tenant_idx').on(table.tenantId),
    index('abandonment_invoice_idx').on(table.invoiceId),
    index('abandonment_triggered_idx').on(table.tenantId, table.recoveryTriggered),
  ]
);

/** Immutable append-only audit log for every recovery action */
export const recoveryAuditLog = pgTable(
  'recovery_audit_log',
  {
    id: varchar('id', { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
    sessionId: varchar('session_id', { length: 36 })
    ...
    actor: varchar('actor', { length: 50 }).notNull().default('recovery_agent'),
...
export type PaymentRetryAttempt = typeof paymentRetryAttempts.$inferSelect;
export type NewPaymentRetryAttempt = typeof paymentRetryAttempts.$inferInsert;
export type PromiseToPay = typeof promiseToPay.$inferSelect;
export type NewPromiseToPay = typeof promiseToPay.$inferInsert;
export type CheckoutAbandonmentSignal = typeof checkoutAbandonmentSignals.$inferSelect;
