...
Write-Host "          RECOVERIQ FREE-TIER INFRASTRUCTURE STATUS        " -ForegroundColor Yellow
Write-Host "========================================================" -ForegroundColor Cyan

$frontendUrl = (terraform output -raw frontend_url 2>$null)
if (-not $frontendUrl) { $frontendUrl = "https://recoveriq-frontend.vercel.app" }

$backendUrl = (terraform output -raw backend_url 2>$null)
if (-not $backendUrl) { $backendUrl = "https://recoveriq-backend.onrender.com" }

$aiUrl = (terraform output -raw ai_service_url 2>$null)
if (-not $aiUrl) { $aiUrl = "https://recoveriq-ai-service.onrender.com" }
