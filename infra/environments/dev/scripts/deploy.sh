...
SSH_KEY="${SSH_KEY_PATH:-$HOME/.ssh/recoveriq-dev}"
...
log "RecoverIQ DEV — Deploying Infrastructure"
...
github_org  = "RecoverIQ-org"
github_repo = "RecoverIQ"
...
ssh -i "$SSH_KEY" $SSH_OPTS ec2-user@"$EC2_IP" "sudo mkdir -p /opt/recoveriq && sudo chown -R ec2-user:ec2-user /opt/recoveriq"

ssh -i "$SSH_KEY" $SSH_OPTS ec2-user@"$EC2_IP" "cat > /opt/recoveriq/docker-compose.yml" << EOF
version: '3.8'

services:
  redis:
    image: redis:7-alpine
    container_name: recoveriq-dev-redis
  ...
  ai-service:
    ...
    container_name: recoveriq-dev-ai-service
    restart: unless-stopped
    ports:
      - "8000:8000"
    env_file:
  ...
  backend:
    ...
    container_name: recoveriq-dev-backend
    restart: unless-stopped
    ports:
      - "3001:3001"
    env_file:
...
ssh -i "$SSH_KEY" $SSH_OPTS ec2-user@"$EC2_IP" "cat > /opt/recoveriq/start.sh" << EOF
#!/usr/bin/env bash
set -euo pipefail

REGION="${AWS_REGION}"
BACKEND_SECRET_ID="recoveriq/dev/backend"
AI_SERVICE_SECRET_ID="recoveriq/dev/ai-service"
ECR_REGISTRY="${ECR_REGISTRY}"
WORK_DIR="/opt/recoveriq"
...
ssh -i "$SSH_KEY" $SSH_OPTS ec2-user@"$EC2_IP" "chmod +x /opt/recoveriq/start.sh && bash /opt/recoveriq/start.sh"
