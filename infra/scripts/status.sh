...
echo -e "\033[1;36m          RECOVERIQ CLOUD INFRASTRUCTURE DASHBOARD         \033[0m"
echo -e "\033[1;36m========================================================\033[0m"
echo ""
...
RDS_STATUS=$(aws rds describe-db-instances --db-instance-identifier "recoveriq-base-postgres" --region "$AWS_REGION" --query "DBInstances[0].DBInstanceStatus" --output text 2>/dev/null || echo "NOT CREATED")
...
echo -e "  • S3 Frontend Bucket:  \033[1;32mACTIVE\033[0m (recoveriq-base-frontend)"
...
DEV_INSTANCES=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=recoveriq-dev-ec2" "Name=instance-state-name,Values=running,stopped" --region "$AWS_REGION" --query "Reservations[0].Instances[0]" --output json 2>/dev/null || echo "null")
...
PROD_CLUSTER=$(aws ecs describe-clusters --clusters "recoveriq-production" --region "$AWS_REGION" --query "clusters[0].status" --output text 2>/dev/null || echo "INACTIVE")

if [[ "$PROD_CLUSTER" == "ACTIVE" ]]; then
  PROD_SERVICES=$(aws ecs list-services --cluster "recoveriq-production" --region "$AWS_REGION" --query "serviceArns" --output json 2>/dev/null || echo "[]")
  SVC_COUNT=$(node -e "const s=JSON.parse(process.argv[1]); console.log(s.length)" "$PROD_SERVICES")
  
  ALB_NAME=$(aws elbv2 describe-load-balancers --names "recoveriq-production-alb" --region "$AWS_REGION" --query "LoadBalancers[0].DNSName" --output text 2>/dev/null || echo "N/A")
  NAT_STATE=$(aws ec2 describe-nat-gateways --filter "Name=tag:Name,Values=recoveriq-production-nat" "Name=state,Values=available,pending" --region "$AWS_REGION" --query "NatGateways[0].State" --output text 2>/dev/null || echo "N/A")

  echo -e "  • Status:               \033[1;32mACTIVE (RUNNING)\033[0m"
  echo -e "  • ECS Cluster:          recoveriq-production (${SVC_COUNT} services active)"
...
if [[ -d "$FT_DIR" ]]; then
  FT_FRONTEND=$(cd "$FT_DIR" 2>/dev/null && terraform output -raw frontend_url 2>/dev/null || echo "https://recoveriq-frontend.vercel.app")
  FT_BACKEND=$(cd "$FT_DIR" 2>/dev/null && terraform output -raw backend_url 2>/dev/null || echo "https://recoveriq-backend.onrender.com")
  FT_AI=$(cd "$FT_DIR" 2>/dev/null && terraform output -raw ai_service_url 2>/dev/null || echo "https://recoveriq-ai-service.onrender.com")
